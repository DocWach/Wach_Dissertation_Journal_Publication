...
DNL
platform:/resource/DissertationModels/src/Models/dnl/ZC.dnl
-1521504746
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class ZC extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public ZC(String name) {
        this(name, null);
    }

    public ZC(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("ZC Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = ZC.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                ZC.class.getResource(".").toString());
        }
