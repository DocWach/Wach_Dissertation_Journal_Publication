...
DNL
platform:/resource/DissertationModels/src/Models/dnl/genC.dnl
-1031289928
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class genC extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public genC(String name) {
        this(name, null);
    }

    public genC(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("genC Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = genC.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                genC.class.getResource(".").toString());
        }
