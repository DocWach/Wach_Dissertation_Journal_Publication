...
DNL
platform:/resource/DissertationModels/src/Models/dnl/ZCG.dnl
-210112700
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class ZCG extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public ZCG(String name) {
        this(name, null);
    }

    public ZCG(String name, Simulator simulator) {
    ...
    public void externalTransition(double timeElapsed, MessageBag input) {
        ...
        if (phaseIs("SG1")) {
            if (input.hasMessages(inIoXG1)) {
                ...
                //ID:EXT:SG1:inIoXG1
                //Add your own code
                if (!input.hasMessages(inIoXG1) || !input.hasMessages(inIoXC1)) {
                    ;
                }
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("ZCG Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = ZCG.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                ZCG.class.getResource(".").toString());
        }
