...
DNL
platform:/resource/DissertationModels/src/Models/dnl/genAC.dnl
-392902700
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class genAC extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    // Output ports
    //ID:OUTP:0
    public final Port<Serializable> outIoXA1 =
        addOutputPort("outIoXA1", Serializable.class);

    //ENDID
    //ID:OUTP:1
    public final Port<Serializable> outIoXA2 =
        addOutputPort("outIoXA2", Serializable.class);
    ...
    public genAC(String name) {
        this(name, null);
    }

    public genAC(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        genAC model = new genAC();
        model.options = options;

        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("genAC Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = genAC.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                genAC.class.getResource(".").toString());
        }
    ...
    public String[] getPhaseNames() {
        return new String[] { "SendX", "SendY", "passive" };
    }
}
