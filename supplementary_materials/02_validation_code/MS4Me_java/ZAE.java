...
DNL
platform:/resource/DissertationModels/src/Models/dnl/ZAE.dnl
2057661182
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class ZAE extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public ZAE(String name) {
        this(name, null);
    }

    public ZAE(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("ZAE Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = ZAE.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                ZAE.class.getResource(".").toString());
        }
