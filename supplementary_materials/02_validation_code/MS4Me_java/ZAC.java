...
DNL
platform:/resource/DissertationModels/src/Models/dnl/ZAC.dnl
2054451708
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class ZAC extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public ZAC(String name) {
        this(name, null);
    }

    public ZAC(String name, Simulator simulator) {
    ...
    public void externalTransition(double timeElapsed, MessageBag input) {
        ...
        if (phaseIs("SA1")) {
            ...
            if (input.hasMessages(inIoXA2)) {
                ...
                //ID:EXT:SA1:inIoXA2
                //Add your own code
                if (!input.hasMessages(inIoXA2) || !input.hasMessages(inIoXC1)) {
                    passivateIn("SA2");
                }
                Serializable variable = messageList.get(0).getData();
        ...
        if (phaseIs("SA2")) {
            ...
            if (input.hasMessages(inIoXA1)) {
                ...
                //ID:EXT:SA2:inIoXA1
                //Add your own code
                if (!input.hasMessages(inIoXA1) || !input.hasMessages(inIoXC1)) {
                    passivateIn("SA1");
                }
                Serializable variable = messageList.get(0).getData();
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("ZAC Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = ZAC.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                ZAC.class.getResource(".").toString());
        }
