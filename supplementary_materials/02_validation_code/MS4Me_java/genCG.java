...
DNL
platform:/resource/DissertationModels/src/Models/dnl/genCG.dnl
562367493
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class genCG extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    // Output ports
    //ID:OUTP:0
    public final Port<Serializable> outIoXG1 =
        addOutputPort("outIoXG1", Serializable.class);
    ...
    public genCG(String name) {
        this(name, null);
    }

    public genCG(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        genCG model = new genCG();
        model.options = options;

        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("genCG Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = genCG.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                genCG.class.getResource(".").toString());
        }
    ...
    public String[] getPhaseNames() {
        return new String[] { "SendX", "passive" };
    }
}
