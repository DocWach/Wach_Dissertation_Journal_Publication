...
DNL
platform:/resource/DissertationModels/src/Models/dnl/genB.dnl
-1821961598
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class genB extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public genB(String name) {
        this(name, null);
    }

    public genB(String name, Simulator simulator) {
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("genB Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = genB.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                genB.class.getResource(".").toString());
        }
