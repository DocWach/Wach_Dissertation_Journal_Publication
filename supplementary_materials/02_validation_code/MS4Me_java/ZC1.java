...
DNL
platform:/resource/DissertationModels/src/Models/dnl/ZC1.dnl
247442966
 Do not remove or modify this comment!  It is required for file identification! */
package Models.java;
...
public class ZC1 extends AtomicModelImpl implements PhaseBased,
    StateVariableBased {
    ...
    public ZC1(String name) {
        this(name, null);
    }

    public ZC1(String name, Simulator simulator) {
    ...
    public void externalTransition(double timeElapsed, MessageBag input) {
        ...
        if (phaseIs("SC1")) {
            if (input.hasMessages(inIoXC1)) {
                ArrayList<Message<Serializable>> messageList =
                    inIoXC1.getMessages(input);
    ...
    public static void main(String[] args) {
        ...
        if (options.isDisableViewer()) { // Command line output only
            Simulation sim =
                new SimulationImpl("ZC1 Simulation", model, options);
    ...
    protected static File getModelsDirectory() {
        ...
        File dir;
        try {
            dirUri = ZC1.class.getResource(".").toURI();
            dir = new File(dirUri);
        } catch (URISyntaxException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Could not find Models directory. Invalid model URL: " +
                ZC1.class.getResource(".").toString());
        }
